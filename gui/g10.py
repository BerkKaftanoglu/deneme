...
class ImageClassifierApp(QMainWindow):
    ...
    def process_image(self, file_path):
        try:
            ...
            # Yeni: Dosya yolu ve tam tarih-zaman
            self.temp_history_entry = (f"{file_info.filePath()} - "
    ...
    def toggle_history_visibility(self):
        if self.right_panel.isVisible():
            ...
            self.toggle_history_btn.setText("▲ Geçmişi Göster")
            self.main_layout.setStretch(0, 1)
        else:
            self.right_panel.show()
            self.toggle_history_btn.setText("▼ Geçmişi Gizle")
